...
def _dataloader_state_payload(train_loader, config, step, batch_size):
...
def _write_dataloader_state_file(state_path, payload):
    data = pickle.dumps(payload, protocol=pickle.HIGHEST_PROTOCOL)

    tmp_path = state_path + ".tmp"
    tf.io.gfile.makedirs(state_path.rsplit("/", 1)[0])
    try:
        with tf.io.gfile.GFile(tmp_path, "wb") as f:
            f.write(data)
        tf.io.gfile.rename(tmp_path, state_path, overwrite=True)
    finally:
        if tf.io.gfile.exists(tmp_path):
            tf.io.gfile.remove(tmp_path)


def validate_dataloader_state_save(train_loader, config, workdir, batch_size):
    """Exercise exact-loader state serialization and GCS write without checkpointing."""
    if not stateful_dataloader_enabled(config):
        return

    state_path = _probe_state_path_for_workdir(workdir)
    payload = _dataloader_state_payload(train_loader, config, step=0, batch_size=batch_size)
    try:
        _write_dataloader_state_file(state_path, payload)
        log_for_0("Dataloader state write probe succeeded at %s.", state_path)
    finally:
...
def restore_dataloader_state(train_loader, config, load_from, zone, expected_step, batch_size):
    ...
    state_path = _state_path_from_checkpoint(checkpoint_path)
    if not tf.io.gfile.exists(state_path):
    ...
    with tf.io.gfile.GFile(state_path, "rb") as f:
