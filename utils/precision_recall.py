...
def _numpy_partition(arr, kth, axis=-1):
    """
    Partial sort to find k-th smallest elements along specified axis.
    This is equivalent to np.partition but works with JAX arrays.
    """
    if isinstance(arr, np.ndarray):
        return np.partition(arr, kth, axis=axis)
    else:
        # For JAX arrays, we need to convert to numpy for partition
        arr_np = np.array(arr)
        partitioned = np.partition(arr_np, kth, axis=axis)
        return partitioned
