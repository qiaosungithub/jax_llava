...
case "${ZONE}" in
    ...
    asia-northeast1-b) BUCKET="kmh-gcp-asia-northeast1-b"; CRED_ZONE="asia-northeast1-b" ;;
    *)
...
gcloud compute tpus tpu-vm ssh "${VM_NAME}" --zone "${ZONE}" \
    --worker=all --project=he-vision-group --command "
pgrep -af python | grep 'main.py' | grep -v 'grep' | awk '{print \"sudo kill -9 \" \$1}' | sh
sudo rm -rf /tmp/tpu_logs
"

echo "[debug_full] launching full pipeline smoke test..."
gcloud compute tpus tpu-vm ssh "${VM_NAME}" --zone "${ZONE}" \
    --worker=all --project=he-vision-group --command "
cd ${STAGEDIR}
echo 'Current dir:'
...
${CONDA_PY_PATH} main.py \
    ...
    --config=configs/load_config.py:remote_debug_full_pipeline
