...
class Output:
  ...
  logits: list[list[list[int]]] | list[list[int]]
  cache: _config.Cache | None
  hidden_states: list[list[list[int]]] | list[list[int]] | None


def _with_sharding_constraint(x, sharding):
    if sharding is None:
        return x
    # sharding 通常是 jax.sharding.Sharding（NamedSharding / SingleDeviceSharding 等）
...
class Transformer(nn.Module):
  ...
  def __call__(  # pytype: disable=signature-mismatch
      ...
      tokens: list[int],
      *,
      images: list[list[list[list[int]]]] | list[list[list[int]]] | None = None,
  ...
  ) -> tuple[list[list[int]], _config.Cache]:
  ...
  def init_cache(
    ...
    return _with_sharding_constraint(cache, sharding)
  ...
  def _merge_mm_embeddings(
      ...
      *,
      tokens,
      embeddings,
      images: list[list[list[list[int]]]] | list[list[list[int]]],
  ):
