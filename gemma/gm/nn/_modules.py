...
class Attention(nn.Module):
  ...
  def setup(self):
    ...
    self.attention_weights = Identity()
  ...
  def __call__(
    ...
    if cache is not None:
      ...
      cache_dtype = cache['v'].dtype

      # [batch_size, cache_size, num_heads, head_dim]
      # Cast to cache dtype so dynamic_update_slice has matching dtypes (e.g. bfloat16 cache vs float32 proj).
      value_proj = jax.lax.dynamic_update_slice(
          cache['v'],
          value_proj.astype(cache_dtype),
          slice_indices,
      )
