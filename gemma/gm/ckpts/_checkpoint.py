...
class LoadCheckpoint:
  ...
  def transform(self, state: HasParams) -> HasParams:  # pytype: disable=signature-mismatch
...
class _CheckpointType(BaseStrEnum):
...
def load_params(
  ...
  if params is None:
    ...
    if sharding is not None:
      ...
      params = _with_sharding_constraint(params, sharding)
  else:
...
def _replicated_sharding():
    # 适用于单进程多设备/多 host：每个 host 自己的本地设备上 replicate
    devices = jax.local_devices()
    return jax.sharding.PositionalSharding(devices).replicate()
